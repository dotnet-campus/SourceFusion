...
{
    /// <summary>
    /// 为 <see cref="ArrayPlaceholder"/> 提供扩展方法。
    /// </summary>
...
";
        ...
        private static Func<ICompilingContext, string> Compile(this ArrayPlaceholder placeholder)
        {
            var builder = new StringBuilder(ClassTemplate)
                .Replace("{parameterName}", placeholder.InvocationParameterName)
                .Replace("{body}", placeholder.InvocationBody);
        ...
        private static string Wrap(this ArrayPlaceholder placeholder, string text)
        {
            switch (placeholder.MethodName)
            {
                case nameof(Placeholder.Array):
                    return $@" new {placeholder.ReturnType}[]
{{
{text}}}";
                case nameof(Placeholder.AttributedTypes):
                    return $"";
                default:
                    throw new MissingMethodException($"{nameof(Placeholder)} 中不包含名为 {placeholder.MethodName} 的方法。");
            }
        }
        ...
        internal static string Invoke(this ArrayPlaceholder placeholder, ICompilingContext context)
        {
            ...
            codeSnippet = placeholder.Wrap(codeSnippet);
            return codeSnippet;
        }
    }
}